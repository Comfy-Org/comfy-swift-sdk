...
**What a run cost.** `result.creditsUsed` carries the `X-Comfy-Credits-Used` Router stamps on a run — Router's own price for that run in Comfy credits, as the decimal string it arrived as (`String` rather than `Double`: binary floating point is the wrong type for a figure you reconcile against money; parse it with `Decimal(string:)` where you need arithmetic). It is a price, not a settled ledger entry — the workspace's usage and invoices remain authoritative. Three things to hold onto: the header is optional and covers a subset of providers, so `nil` means **not reported** and never *free*; a replay is not stamped with it, so a replayed response reports no figure even though the original call had a cost (and, being a replay, is not charged again); and `"0"` is a real reported cost of nothing — branch on `if let creditsUsed`, never on the value being non-zero.
