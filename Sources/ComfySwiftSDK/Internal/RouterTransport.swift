...
internal actor RouterTransport {
    ...
    /// in flight is a different mechanism than a floor: that is
    /// ``withWallClockDeadline(_:operation:)``'s job rather than this one's.
    ///
    ...
    ///
    /// This is the wall-clock stop the public `timeout` documents. `collect`'s guards bound
    ...
    ///
    /// The timer sleeps *until the instant*, on the same ``ContinuousClock`` the deadline was
    /// taken from, rather than for a converted count of nanoseconds. That is what keeps the
    /// two halves of the bound on one monotonic clock — the timer cannot drift away from the
    /// guards in `collect` — and it removes the conversion entirely: `Task.sleep(nanoseconds:)`
    /// takes a `UInt64`, and `UInt64(_:)` **traps** rather than throwing on a large `Double`.
    /// A deadline already in the past needs no special case: `sleep(until:)` returns at once,
    /// so the race resolves to `.expired` — and `collect`'s own guard throws before any request
    /// is built, so nothing billable is sent either way.
    ///
    ...
    private func collect(
        ...
        while true {
            ...
            // three locks on the same door, not the door: the guards above bound the call
